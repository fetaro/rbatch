...
# $Id: cmd_spec.rb 683 2012-04-24 07:23:26Z lki-q4-yang $
# Author:: LKI 朱
  ...
  describe 'cmd' do
    describe '*cmd_params' do
      ...
      it 'status 256以上(Linuxならばmod256になるのでNG)' do
        returncode = 300
        str = "ruby -e 'STDOUT.print 1; STDERR.print 2; exit #{returncode};'"
        stdout, stderr, status = IAX.cmd(str)
        stdout.chomp.should == "1"
        stderr.chomp.should == "2"
        case RUBY_PLATFORM
          when /mswin|mingw/
            status.should == returncode
          when /cygwin|linux/
            status.should == returncode % 256
        end
      end
    end
